...
def Weight_Loss():
    ...
    df = data.iloc[DinnerfoodseparatedID]
    ...
    ## train set - lay ra cac chat can thiet cho moi diet plan
    dataTog=datafin.T
    bmicls=[0,1,2,3,4]
    agecls=[0,1,2,3,4]
    weightlosscat = dataTog.iloc[[1,2,7,8]]
    # print(weightlosscat) -> weightloss food nutrion: 1Fats 2Proteins 7Carbohydrates 8Fibre
    weightlosscat=weightlosscat.T # dao nguoc row->col va col->row
    weightgaincat= dataTog.iloc[[0,1,2,3,4,7,9,10]] # weightgain food nutrion: 0Calories 1Fats 2Proteins 3Iron 4Calcium 7Carbohydrates 9VitaminD 10Sugars
    weightgaincat=weightgaincat.T
    healthycat = dataTog.iloc[[1,2,3,4,6,7,9]] # healthy food nutrion: 1Fats 2Proteins 3Iron 4Calcium 6Potassium 7Carbohydrates 9VitaminD
    healthycat=healthycat.T
    weightlosscatDdata=weightlosscat.to_numpy() #chuyen thanh Numpy array
    weightgaincatDdata=weightgaincat.to_numpy()
    healthycatDdata=healthycat.to_numpy()
    weightlosscat=weightlosscatDdata[1:,0:len(weightlosscatDdata)]  #bo di row dau tien trong ket bang ket qua (160 calo)
    weightgaincat=weightgaincatDdata[1:,0:len(weightgaincatDdata)]
    healthycat=healthycatDdata[1:,0:len(healthycatDdata)]
    
    # them 2 col moi vao food distribution nutrion
    weightlossfin=np.zeros((len(weightlosscat)*5,6),dtype=np.float32) #Tạo mảng hai chiều các phần tử 0 với kích thước len(weightlosscat)*5 x 6. (6 column)
